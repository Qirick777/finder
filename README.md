...
| `Kinship` | 근친 회피(형제 + 직계 존비속 전체 — 조부모·증조 포함 · 사촌 등 방계 허용) — 자식이 조상 명단(부모+양가 병합)을 영구 보유해 중간 세대가 죽어도 판정 유지 (§13-E) |
...
- **미믹 특성 검사봉**("도구" 탭): 미믹 우클릭 → 정보 표시. **쉬프트+스크롤로 모드 전환**:
  ...
  · **쉬프트+우클릭 = 가계도 GUI**: 클릭한 미믹 + 조상 3세대를 상자·연결선 트리로 표시.
  조상 상자를 클릭하면 그 조상이 새 포커스가 되어 **위로 무한히 거슬러 올라감**(죽은 조상도 혈통
  원장으로 표시 — N번호·세대·출생/사망일·자식/후손 수, 미상은 회색 `?`).
...
- **신규 기능 직접 점검**(상황 일보직전 세팅 → 눈으로 확인):
  ...
  - 식량 v2 즉시 판별 명령(전부 로그 자동 ON, "결과 직전" 상태로 시작, **수치 변화 자동 판정**):
    - **`/evosim trip`** — 남편 H=2.5·아내 H=0.7+저장고3. 2초마다 H·저장고 수치 중계 → 남편 H∈[1,2)(입금)·아내 H≥1.5(인출)면 ✅ 자동 판정(30초 초과 ❌)
    - **`/evosim share`** — 아내 위급 0.25·남편 여유 2.0·저장고0. 수치 중계 → 아내 H≥0.6·남편 H≤1.8이면 ✅(20초 초과 ❌)
    - **`/evosim birth`** — 저장고 20 + 즉시 정산 → **명령 직후** "저장고 20→기대치(17−베리비용)·유아 1명(기대 1) ⇒ ✅/❌" 수치 판정 출력. 베리도 저장고를 **실제 차감**하므로 심은 그루 실측으로 회계 대조
    - **`/evosim care`** — 유아 H0.5+저장고5 + 즉시 정산 → **명령 직후** "유아 H 0.50→1.50·저장고 5→4 ⇒ ✅/❌" 수치 판정 출력
    - **`/evosim r6`** — 밤에 위급 2명: 저장고 없는 쪽 채집 강행 / 있는 쪽 귀가 [인출] 분기 판별
    - **`/evosim suitor`** — 64블록 두 마을 + 고립 남성 외로움 강제 → 즉시 [구혼여행] 출발→[짝성립] 판별
    - **`/evosim polygamy`** — 중혼(일부다처): 관용 아내 부부+저장고 40(부양선 18 여유 — 감시 중 출산·베리 자연 차감 대비)+독신 여성 → 신부 혼인·합류 수치 판정
    - **`/evosim elder`** — 노년 전이·자연사: fast 성장 성년이 노년 경유 후 사망하는지 상태 실측(~5초)
    - **`/evosim eldercare`** — 노인 공유·마실: 책임 노인(잉여 3.5)이 친자식 집(유아)에 배달 → **그 집 저장고 0→2** 수치 판정
  - **`/evosim checkall`** — ★ 원트랙 일괄 검증: 위 전부+중혼 거절+**노년 4종(전이·공유·무공유·마실)**까지 **60단계를 한 명령으로**(㊳ 순수 evotest 전량 + ㊴~ 밭 게이트 18종 전 편입, 단계별 플레이어 슬롯 동행 이동)(+집앞 인출·아들 동거 번식·전투 해제·이주 가드 2종·낮샘플 육아 2종·**원장 등록/무대 제외·날로먹기 2종·힘 공격 속성 2종·무모 번식 완화+대조군·조심성 유인+대조군**) 자동 조성→결과값 판정→✅/❌ 요약(VerifySuite). **판정마다 영문 1줄을 cmd 콘솔(성공 녹/실패 적)과 `evosim-verify.log`에 기록** — 파일을 그대로 붙여넣으면 AI가 슬러그·reason·수치로 원인 역추적 가능
- **인구 통계·혈통** (혈통 원장 = 등장한 모든 개체의 영구 기록. 무대 명령 개체는 제외):
  - **`/evosim stats`** — 통계 GUI: 생존 개체의 **발현 특성 분포 가로 막대그래프**(최다→최소, 보유자
    수·비율 — 한 개체가 여러 특성을 가지므로 합계 100% 아님) + **최다 후손 랭킹**(죽은 조상 포함,
    다이아몬드 혈통 중복 제거). 랭킹 행 클릭 → 그 개체의 가계도로 이동
  - **`/evosim legacy`** — 같은 후손 랭킹을 채팅 텍스트로(콘솔·로그 대조용 폴백)
- **봉건 밭 경제**(M0~M8): 잉여 → 독립 밭 개간(재배줄+고랑 수열, 티어 9→49) → 밭이 가구 수확력
  (C=12/인)을 넘으면 **일자리 슬롯** 게시 → 소작(수확 70% 본인·30% 지대) → 3일 연속이면 **상시
  소작**(예약석) ↔ 영주의 위급 구제 의무(불이행 시 해제) → **확장권은 소작에게**(주인은 신규 밭만,
  체증 비용) → 능력 게이트(채집·저장 능력 없으면 35타일 상한) → 장자 상속·무주지 선점 → **만족
  규칙**(잉여가 기준 초과면 노동 정지 — 부지런·경쟁·야망가·욕심·사치만 계속). 관측:
  `/evosim lords`(지주 랭킹), 검사봉 HOME 모드(구획·타일·지대·소작·만족), verify.log 영문 판정.
  검증 명령: farm/farmown/farmhire/farmguard/farmrent/farmbond/farmseat/farmshield/farmbreak/
  farmgrow/farmfound/farmcap/farminherit/farmvacant/farmidle/farmdrive(전부 결과값 자동 판정,
  핵심 4종은 checkall ㉞~㊲ 편입)

...
- **`/evolog on`** — 기록 시작(파일 열림). **`/evolog off`** — 종료.
- **`/evolog dump [n]`** — 최근 n건(기본 30)을 채팅에 출력.
- **`/evolog census`** — 지금 즉시 인구 스냅샷을 로그에 남김(로그 켜진 상태에서 매일 자동으로도 1회 기록).
- **`/evolog clear`** — 메모리 버퍼 비움.
- 기록되는 사건: `짝성립` · `출산` · `급식`/`방치`/`아사`(유아 돌봄) · `성장`(단계 전환) ·
  `전투`(교전/도주/퇴각/복귀) · `사망`(원인 포함) · `CENSUS`(인구·세대 스냅샷).
- 각 줄 형식: `[day3 밤] 출산   #42(여·성년) @12,64,-8 자식 #57 세대2 · 부친 #40 (누적 1)`.

...
- **신규 기능 직접 점검**(상황 일보직전 세팅 → 눈으로 확인):
  ...
  - **`/evosim checkall`** — ★ 원트랙 일괄 검증: 위 전부+중혼 거절+**노년 4종(전이·공유·무공유·마실)**까지 **37단계를 한 명령으로**(+집앞 인출·아들 동거 번식·전투 해제·이주 가드 2종·낮샘플 육아 2종·**원장 등록/무대 제외·날로먹기 2종·힘 공격 속성 2종·무모 번식 완화+대조군·조심성 유인+대조군**) 자동 조성→결과값 판정→✅/❌ 요약(VerifySuite). **판정마다 영문 1줄을 cmd 콘솔(성공 녹/실패 적)과 `evosim-verify.log`에 기록** — 파일을 그대로 붙여넣으면 AI가 슬러그·reason·수치로 원인 역추적 가능

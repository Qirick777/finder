...
- [~] **Phase 4 — 사회 (거처·짝짓기·번식·이주)**
  ...
  - [x] **짝짓기 배회·구애(후보 줄세우기)**: 이성 방랑자 후보를 **매력 순으로 줄 세워 위에서부터** 시도.
        · 배회 시간대엔 넓게(48블록) **적극적으로 다가가** 시도, 노동 시간대엔 아주 가까울 때만 스치듯 시도
        · 접촉하면 조우 판정 **1회** → 성립이면 정착, 실패면 그 상대는 포기(쿨타임)하고 다음 후보로
          (**졸졸 안 따라다님**, 못 따라잡으면 포기)
        · 거절/컷이면 눈을 낮춤(간격 제한으로 급락 방지) → 반복될수록 까다로움↓ → 결국 성립.
          호환쌍은 즉시·비호환쌍은 수렴(교착 없음), `/evotest mating`(수렴)으로 검증

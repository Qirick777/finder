...
public final class FarmEconomy {
    ...
    // ── 작물 관리(밭 노동의 두 번째 얼굴) ──────────────────────────────────────────
    //
    // 출근한 소작이 익은 타일을 다 따고 나면 종전에는 그냥 놀았다(실측 가동률 72% — 하루
    // 5.8타일만 따고 용량 8 중 나머지를 버림). 그 시간을 밭을 돌보는 데 쓰게 하고, <b>돌보는
    // 동안에만</b> 익음이 빨라진다. 상태를 저장하지 않는다 — 지금 밭에 누가 있는가만 본다.

    /** 미믹 한 명이 돌볼 수 있는 타일 수(등급 Ⅰ 기준). 100타일 밭 = 약 4.2명이면 만석. */
    public static final double CARE_BASE = 24.0;

    /** 관리등급 한 칸당 케어 범위 증가율 — Ⅰ 24 · Ⅲ 36 · Ⅴ 48. */
    public static final double CARE_GRADE_STEP = 0.25;

    /**
     * 커버리지 1.0 에서의 익음 가속 — 관리 중 성장률 = 1 + 이 값 × 커버리지.
     *
     * <p><b>하루 산출이 이 배수만큼 되는 것이 아니다.</b> 관리는 근무 구간(1000~8000틱, 하루의
     * 29%)에만 일어나므로, 2.0 이면 하루 진행 = (24000−T) + 3T = 24000 + 2T 이고 근무 구간을
     * 전부 관리에 써도 1.58회/타일/일(+58%)이다. 절반을 수확에 쓰면 +29% 다. 2배를 원하면
     * 3.4 가 필요한데, 우선 2.0 으로 두고 고용·노동이 제대로 도는지부터 본다(지시).
     */
    public static final double CARE_MAX_BOOST = 2.0;

    /** 이 개체가 돌볼 수 있는 타일 수 — 성중립(관리등급). 채집배율을 쓰면 성별이 섞인다. */
    public static double careRange(Individual ind) {
        int g = Multipliers.manageAbilityGrade(ind);
        return CARE_BASE * (1.0 + CARE_GRADE_STEP * Math.max(0, g - 1));
    }
     ...
     */
    /* 8 → 12 를 시도했다가 되돌렸다(런 12 실측, d8 중단). 근거였던 "용량 소진(8/8)으로 코앞 익은
     * 타일을 안 딴다"(하루 15~44건)는 사실이었지만, 결과는 반대였다: 용량 12 면 지주 부부(24)가
     * 24타일 밭을 혼자 다 따서 부족분 게시가 0 이 되고 소작이 안 뽑힌다(소작 출근 1~2명 vs 런 11
     * 20~29명). 소작 임금이 사라져 평민 저장고 5~8(런 11: 15~20), 구획 계정(소작 수확 몫)이 0 이라
     * 36타일 이후 재투자 확장이 멈췄다(d5 36 → d8 40, 런 11 d8 107). 즉 이 값은 "누가 따느냐"를
     * 정하는 손잡이이고, 올리면 산출이 소작·지대에서 지주 자가 수확으로 옮겨 가 봉건 고리가
     * 끊긴다. 익은 타일 방치를 줄이려면 용량이 아니라 지주 자가 몫(shortfall 의 ownCap)을
     * 손봐야 한다 — 별건. */
    ...
    /**
     * 1.0 → <b>0.4</b> (진행 속도 — 플레이어가 봉건 구조를 보기까지의 시간).
     *
     * <p>고칠 것은 균형이 아니라 <b>시계</b>다. 실측(w10)에서 밭이 12타일(d2)에서 100타일(D20)이
     * 되기까지 18 게임일이 걸렸고, 마크 하루가 실시간 20분이므로 <b>계층이 눈에 보이기까지
     * 약 7시간</b>이었다. 초기 밭벌이가 하루 1.5~6 인데 타일당 1.5~3.4 를 물어야 해서 하루
     * 1~3타일밖에 못 늘린 것이 원인이다.
     *
     * <p>수확 계수·출산 문턱·만족선은 <b>건드리지 않는다</b>. 지금까지 맞춰 온 균형(지배자
     * 출산 4.0 · 마름 3.0 · 군인 5호)을 그대로 두고 성장 구간만 압축한다.
     */
    public static final double EXPAND_COST = 0.4;

    /**
     * 단계마다 개간이 비싸지는 비율 — 타일당 비용 = EXPAND_COST × STAGE_COST_MULT^(단계−1).
     *
     * <p>1.5 → <b>1.3</b>. 1.5 는 후반 단계에서 기하급수로 뛰어(6단계 7.6/타일) 큰 영지일수록
     * 더 느려졌다 — 정작 볼 것이 많아지는 구간에서 시계가 가장 느렸다.
     *
     * <p><b>깨지는 결합 하나를 명시한다.</b> 아래 주석이 적어 둔 "3단계 총액 27 이 만족선
     * 24~27.6 사이에 걸려 평민은 거기서 멈춘다"는 잠금은 이 값 변경으로 사라진다(총액 27 → 8).
     * 그래도 계층은 유지된다 — 평민 밭의 1차 잠금은 비용이 아니라 {@link #plotTileCap} 이고
     * (추종자 0명 → 12타일 = 2단계), 비용 곡선은 그 위에 얹힌 2차 잠금이었다. 이 판단은
     * 추정이므로 <b>확장 가속 후 평민 밭이 12타일을 넘는지</b>를 실측으로 확인할 것.
     */
    public static final double STAGE_COST_MULT = 1.3;
     ...
     *
     * <p><b>12 → 24</b>(사용자 승인, 런 23 실측): 추종자 0 이면 12칸인데 12칸은 부부(용량 16)가 다
     * 따서 소작 자리가 안 나고, 자리가 없으니 신세(상시 0.6/일)가 안 쌓여 추종자가 0 에 머문다.
     * 이 고리를 깨던 것은 초반 나흘 안에 누가 굶어 긴급고용(신세 5)되는 우연뿐이었다(런 16~22 는
     * 전부 1~6건, 런 23 은 0건 → 군주 밭 d7 31칸 vs 다른 런 56~114). 24칸 = 부부 16 + 일꾼 한 자리
     * 8 — 굶는 사람이 없어도 첫 소작 자리가 난다. 추종자 0 → 24 · 1 → 36 · 2 → 48.
     */
    public static final int PLOT_TILE_BASE = 24;
    ...
    /*  12→24(진행 속도): 비용을 4배 낮추면(EXPAND_COST·STAGE_COST_MULT) 자금 제약이 풀려
     *  <b>이 노동 캡이 새 병목</b>이 된다 — 캡 12 면 12타일에서 100타일까지 여전히 최소 8일이
     *  걸려 압축 효과의 절반이 사라진다. 능력 경사는 그대로다: 실효 확장은 여전히
     *  소득×재투자몫÷비용 이 정하고, 캡은 그 위의 천장일 뿐이다. */
    public static final int EXPAND_PER_DAY = 24;
     ...
     */
    /**
     * 1.0 → <b>1.3</b> (소작 출산이 목표에 <b>늦게</b> 닿는 문제).
     *
     * <p>수치가 낮은 것이 아니라 늦다. 실측 추이에서 소작·농노 출산이 1.0 에 닿는 시점이
     * 늘 D19~D20 이었다(w9 D19 1.00 · w10 D19 0.80 · p1 D10 0.56). 마크 하루가 실시간 20분이라
     * <b>플레이어가 계층 재생산을 보기까지 6~7시간</b>이다. 지배자는 D7 에 이미 3.0 을 찍는데
     * 소작만 이렇게 늦다.
     *
     * <p>확장을 빠르게 해도 이 격차는 안 줄었다 — 밭이 커지면 소작도 같이 늘어 1인당 몫이
     * 제자리이기 때문이다(p1 D9: 51타일인데 1인당 수확 2.8). 그래서 면적이 아니라 <b>1인당
     * 산출</b>을 올린다.
     *
     * <p>폭은 산술로 정했다. 소작 소득 2.7 − 소모 1.8 = 순저축 0.9/일. 1.3 이면 소득 3.51,
     * 순저축 1.71 로 <b>1.9배</b> — 출산이 D10 경 1.0 에 닿고 D20 에 약 2.6 으로 목표(1~3) 안에
     * 머문다. 1.5 는 순저축 2.5배라 D20 에 3 을 넘길 위험이 있어 택하지 않았다.
     *
     * <p>지배자 출산이 함께 오르는 것은 제약이 아니다(지시: "엘리트의 출산률은 더 높아도 된다").
     * 소작 몫(fee)을 올리는 대안은 지주 몫을 깎아 군인·시설을 늦추므로 택하지 않았다.
     */
    public static final double TILE_YIELD_MULT = 1.3;
    ...
    /**
     * <b>자립 규모</b> — 이 크기부터 밭은 제 계정만으로 자란다(그 아래는 지주 저장고가 받친다).
     *
     * <p>종전 기준은 "상시소작이 하나라도 있는가"였다. 그것은 밭의 <b>크기를 전혀 보지 않는</b>
     * 이분법이라, 10타일짜리 밭에 소작 한 명이 붙는 순간 지주 지원이 끊겼다. 실측(사용자 월드):
     * 12타일 · 계정 3.1 · 지주 저장고 60 → 계정 30%인 0.93 만 쓸 수 있어 <b>하루 1칸</b>.
     * 자립 못 하는 크기에서 자립시킨 것이다.
     *
     * <p>수를 실측으로 잡았다. 계정만으로 하루에 넓히는 칸수 = 계정 × {@link #MATURE_REINVEST_SHARE}
     * ÷ {@link #expandCost}:
     *
     * <pre>
     *   12타일  계정  3.1(실측) ÷ 0.52 → 1칸/일   ← 사실상 정지
     *   24타일  계정  8  (외삽) ÷ 0.68 → 3칸/일   ← 손익분기
     *   36타일  계정 20  (외삽) ÷ 0.88 → 6칸/일   ← 여유
     *   49타일  계정 41.4(실측) ÷ 1.14 → 10칸/일  ← 실측과 일치
     * </pre>
     *
     * <p>손익분기는 24 근처지만 36을 쓴다. 위 계정 값이 세 점에서 뽑은 외삽이라, 24 는 오차가
     * 아래로 벗어나면 곧장 1~2칸/일로 주저앉는 자리다. 36 은 밭 단계 경계이기도 하다
     * (TIERS 6·12·24·36·54) — 소작 4~5명이 각자 용량 8타일을 채우고도 남는 규모다.
     *
     * <p>새 밭이 지주 지갑을 계속 빨아가지 않는가: 지갑이 열린 동안 확장 속도는 노동 상한
     * ({@link #EXPAND_PER_DAY} 24칸/일)까지 나오므로, 밭 하나가 이 선을 넘는 데 <b>1~2일</b>이면
     * 된다. 밭당 이틀치가 나가는 것이지 무한정 빨리는 것이 아니다. 게다가 다음 밭을 여는 데는
     * 별도 자격(최신 밭 {@link #MATURE_TILES} 이상 + 상시소작 1명 이상)이 걸려 있다.
     */
    public static final int SELF_FUNDING_TILES = 36;

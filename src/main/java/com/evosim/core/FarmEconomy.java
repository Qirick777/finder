...
public final class FarmEconomy {
     ...
     */
    /* 8 → 12 (사용자 승인, 런 11 실측): "코앞에 익은 타일이 있는데 하루 수확 용량 소진(8/8)으로
     * 안 딴다"가 하루 15~44건, 밭이 게시한 미충족 청구 43~89 — 병목의 절반은 땅이 아니라 사람당
     * 상한이었다. 같은 소작이 50% 더 따서 소작 수취가 먼저 오르고(3.3 → 5 안팎), 지대는 그에
     * 비례한다. 부지런·게으름·노년 배율은 그대로. */

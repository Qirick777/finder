...
public final class FarmLayout {
    ...
    /** 다음 단계가 덩어리를 늘리는가(아니면 줄을 늘린다) — 표준 수열의 s번째 기준. */
    public static boolean nextAddsBed(int s) {
        return stage(s + 1)[0] > stage(s)[0];
    }

    /**
     * <b>이 모양에서 다음 수</b> — 덩어리를 늘릴 차례인가(아니면 줄).
     *
     * <pre>줄 &lt; 3×덩어리 면 줄을 늘리고, 아니면 덩어리를 늘린다.</pre>
     *
     * <p>단계 <b>번호</b>로 정하면 안 된다. 한쪽이 막혀 대체 수를 쓰면 (2,9) 처럼 표준 수열에
     * 없는 모양이 되는데, 번호로 찾으면 그때부터 "수열 밖"이 되어 <b>영영 자라지 못한다</b>
     * (실측: 한쪽만 막힌 자리에서 3단계 뒤 정지). 모양 자체로 정하면 이탈해도 복귀한다 —
     * (2,9) 는 줄 9 ≥ 3×2 이므로 덩어리를 늘려 (3,9) 로 돌아온다.
     *
     * <p>표준 수열은 이 규칙의 특수해다: (1,3)→(2,3)→(2,6)→(3,6)→(3,9)→(4,9)…
     */
    public static boolean addBedNext(int beds, int rows) {
        return rows >= ROW_STEP * Math.max(1, beds);
    }
}

...
public final class FoodEconomy {
     ...
     *
     * <p><b>1.5(원래 값)로 되돌아와 있다.</b> 2.2 를 시도했다가 아이가 너무 깎여 1.8 로
     * 낮췄고, 그 런에서 엘리트가 6일째 밭을 못 세우자 <b>"개간 예비도 가구 소모 비례라
     * 사슬이 끊긴다"고 결론지었는데 그 설명은 틀렸다.</b> 착공 판정이 걸린 d0 밤에는
     * <b>소년이 존재하지도 않는다</b>(d0 출생 유아가 d1 에 소년이 된다). 소년 소모가
     * 원인일 수 없다.
     *
     * <p>진짜 원인은 확인하지 못했다 — 그 런의 로그를 다음 런이 덮어썼다. 그리고 애초에
     * 비교 자체가 성립하지 않았다: {@code level-seed} 는 <b>지형만</b> 고정하고
     * {@code evosim wildpairs} 가 뽑는 18명의 특성·짝짓기·위치는 매 런 새로 뽑힌다.
     * 런마다 엘리트가 다른 사람이었다(헨리 엘리엇 · 에런 워커 · 에머리 블랙우드 ·
     * 핀리 오스본 · 윌리엄 딕슨 · 오스카 포스터). <b>같은 조건의 A/B 가 아니었다.</b>
     *
     * <p>그러므로 아이 소모에 대해 지금 말할 수 있는 것은 <b>2.2 에서 인구·출산이 낮게
     * 나온 한 번의 관측</b>뿐이고, 그것도 통제된 비교가 아니다. 다시 다루려면 소환 난수를
     * 고정하거나 같은 설정을 여러 번 돌려야 한다.
     *
     * <p>이전 시도의 실측(참고) — 2.2(시드11 d8, 유지비 1.5 위에서):
     * <pre>
     *   인구 52 → <b>43</b> · 미성년/성인여 0.70~0.78 → <b>0.39</b>   (아이를 너무 깎았다)
     *   무토지 9.9 → <b>6.5</b> · 유지비 못냄 1건 → <b>12건</b>       (빈민화는 노린 대로)
     * </pre>
     * "첫 출산은 안 막히니 둘째만 완만히 무거워진다"고 보았는데, 둘째 억제가 예상보다 훨씬
     * 세게 걸렸다. 잉여를 깎은 것보다 아이를 더 깎았으므로 성인 소모와 같은 실패 모양이다.
     *
     * <p>성인 2인 가구 출산선: 자녀 없음 12.0 <b>불변</b> · 소년1 13.5→13.8 · 소년2 15.0→15.6.
     *
     * <p><b>d8 로만 비교하면 위험하다</b>: 같은 런에서 d8 타일 102·지주 85.9·저장고합 202 가

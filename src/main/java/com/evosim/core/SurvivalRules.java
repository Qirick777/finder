...
public final class SurvivalRules {
    ...
    public static double moveSpeedFactor(LifeStage stage) {
        return switch (stage) {
            ...
            // 통근으로 끝나 배울 시간이 남지 않아 0.8 로 올렸고, 통학 한계를 48 → 96 으로
            // 넓히면서 <b>0.95</b> 로 한 번 더 올린다. 96블록이면 등굣길이 0.8 에서 약 690틱
            // (노동 8000틱의 8.6%)인데, 0.95 면 580틱으로 줄어 먼 아이도 앉아 있을 시간이
            // 남는다. 성인(1.0)을 넘기지는 않는다 — 아이가 어른보다 빠를 이유는 없다.
            case BOY -> 0.95;
            case ADULT -> 1.0;

...
public final class FarmTicker {
    ...
    private static void growFarms(ServerLevel level) {
        ...
        for (FarmStore.Plot plot : expandOrder) {
            ...
            // 부트스트랩(자영) — 저장고 예비 위 잉여를 폴백 재원으로. <b>원본 조건 그대로다.</b>
            //
            // 한 번 무조건 켜 봤다가 되돌렸다. 항상 켜면 지주 저장고가 매일 예비선(12)까지
            // 깎여 축적이 불가능해진다 — 시설 문턱(막사 44~144 · 교회 66 · 학교 76)에 영영
            // 못 닿고, "이름만 지주"가 된다. 소작이 붙은 밭이 계정으로만 크는 대신 지대의
            // 70%가 저장고로 흘러 지주가 600까지 쌓이는 쪽(실측)이 이 프로젝트가 원하는 그림이다.
            //
            // 밭 성장이 느려지는 것은 이 조건 탓이 아니라 <b>승격이 빨라진</b> 탓이다 —
            // 긴급고용 승격(2923596)이 상시소작이 붙는 시점을 앞당겨, 밭이 어릴 때 이미
            // nTen > 0 이 된다. 손댈 곳이 있다면 그쪽이지 여기가 아니다.
            boolean bootstrap = nTen == 0;
            if (bootstrap) {

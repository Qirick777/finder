...
public final class FarmTicker {
    ...
    private static void growFarms(ServerLevel level) {
        ...
        for (FarmStore.Plot plot : expandOrder) {
            ...
            // <b>확장 재원에 지주 저장고를 항상 연다.</b>
            //
            // 종전 조건은 nTen == 0 이었는데, 승격 경로가 고장나 상시소작이 구조적으로 0이라
            // <b>실제로는 모든 밭이 항상 부트스트랩</b>이었다. 관리 시절의 밭 성장 속도는 그
            // 동작에서 나온 것이다. 승격을 고치자(긴급고용 연속 카운터) 그 조건이 처음으로
            // 살아나 24타일 근처에서 자금이 끊기고 성장이 주저앉았다 — 죽어 있던 제동이
            // 갑자기 걸린 것이지, 새 제동이 생긴 것이 아니다.
            //
            // 되살릴 이유가 없다. 지주 축장은 <b>다른 손잡이</b>가 이미 지킨다: 재투자 캡
            // (MATURE_REINVEST_SHARE 30%)이 계정 쪽을 묶고, expandReserve 가 저장고에 바닥을
            // 깔아 지주가 예비금 아래로는 내려가지 않는다. 소작 유무로 재원을 통째로 끊는 것은
            // 그 위에 겹쳐진 세 번째 제동이었고, 그것 하나가 규칙4(밭의 지속 성장)를 눌렀다.
            //
            // 변수로 남겨 두는 것은 아래 지불 조건이 같은 판정을 봐야 하기 때문이다 — 재원에
            // 저장고를 더해 놓고 차감을 건너뛰면 타일이 공짜로 생긴다(감사 항등식 파괴).

...
public final class FarmTicker {
    ...
    private static void growFarms(ServerLevel level) {
        ...
        for (FarmStore.Plot plot : expandOrder) {
            ...
            // 부트스트랩은 <b>어린 밭에도</b> 열어 준다 — 종전 조건(nTen == 0)에 OR 로 <b>더한다</b>.
            //
            // 종전 조건이 오래 맞아떨어진 이유는 승격 경로가 고장나 상시소작이 구조적으로 0이라
            // 어린 밭에 소작이 붙는 상황 자체가 없었기 때문이다. 승격을 고치자(긴급고용 연속
            // 카운터) 그 전제가 깨졌다: 10타일짜리 밭에 소작 하나만 붙어도 다 큰 밭 취급을 받아
            // 지주 지원이 끊기고, 계정 30%(실측 5.41×0.3 = 1.6 → 3칸)만 남아 d4·d5 에 +2씩
            // 넣고 멈췄다(실측 확장 이력).
            //
            // 그 회귀만 되돌린다. nTen == 0 갈래는 <b>그대로 둔다</b> — 소작 없는 성숙 밭의
            // 거동을 바꾸는 것은 이 결함과 무관하고, 실측 근거도 없다. 조건을 통째로 성숙
            // 기준으로 갈아치우면 아무도 요청하지 않은 변경이 함께 들어간다.
            //

...
public final class HomeBlueprint {
    ...
    private static HomeBlueprint legacy(BlockPos home, byte facing2d) {
        ...
        //
        // 상한은 <b>칸 수와 다르다</b>. 천막의 gardenCells 는 기본 8칸 + 지형 폴백 8칸 = 16이고,
        // 폴백은 "기본 칸이 막혔을 때 대신 쓸 자리"이지 늘어난 정원이 아니다. 상한을 칸 수로
        // 잡으면 구 천막 가구의 정원이 8 → 16으로 두 배가 되어 식량이 부풀고, 그 위에 세워진
        // 밭·지대·출산 회계가 전부 어긋난다(검증 스위트의 저장고 기대값이 이걸 잡아냈다).
    ...
    /**
     * 정원 그루 상한 — <b>칸 수와 같지 않을 수 있다</b>. 천막은 16칸을 훑되 상한은 8이다
     * (뒤쪽 8칸은 앞 칸이 지형에 막혔을 때 쓰는 대체지). 스키메틱은 칸 수 = 상한.
     */

...
public final class HomeTemplate {
    ...
    public enum Tier {
        SMALL(4, 0.0, 1.5, "small1", "small2", "small3"),
        MIDDLE(6, 12.0, 2.8, "middle1", "middle2"),
        BIG(9, 30.0, 4.5, "big1", "big2"),
        ...
        MANSION(12, 70.0, 7.5, "mansion", "mansion2");
         ...
         *
         * <p><b>0.05/0.15/0.4/1.0 → 1.0/2.0/3.5/6.0.</b> 종전 값은 죽은 손잡이였다:
         * 저장고는 정수 유닛만 걷는데 소형이 0.05/일이라 <b>20일에 한 번</b> 1이 빠진다.
         * 실측(시드11, 12일 런) — 마을 전체에서 유지비가 걷힌 것이 <b>딱 1건</b>(그것도 저택)이었다.
         *
         * <p>새 눈금의 근거는 <b>집 정원</b>이다. 정원 산출이 가구당 하루 1~2.5 인데 그것이
         * 지대도 고용주도 없는 무조건 소득이라, 무토지 가구를 9.7 에 붙들어 놓고 아무도 절박하지
         * 않게 만들고 있었다. 소형 1.0 은 그 정원 몫을 절반~전부 상쇄한다 — 노동 소득이 있는
         * 소작에게는 소득의 20~30%지만 정원 하나로 사는 가구에는 <b>67%</b>다. 균일한 값이
         * 소득 구조 때문에 누진으로 작동한다.
         *
         * <p><b>표본 시각을 반드시 그날 마지막 AUDIT 으로 잡을 것.</b> 한때 2차(1.5)를
         * "출산이 흔들린다"고 판정해 1.2 로 후퇴시킨 적이 있는데, 그 근거가 <b>하루 중간</b>
         * 표본이었다. 지대가 밤에 정산되므로 같은 d5 안에서도 값이 이만큼 움직인다:
         * <pre>
         *   2차(1.5) d5 중간  인구 38 · 미/여 0.57 · 1인당 3.6 · 지주 <b>31.8</b>
         *   2차(1.5) d5 끝    인구 40 · 미/여 0.71 · 1인당 4.9 · 지주 <b>80.8</b>
         *   1차(1.0) d5 끝    인구 41 · 미/여 0.69 · 1인당 4.3 · 지주 55.6
         * </pre>
         * 끝시각으로 보면 1.5 는 1.0 보다 나쁘지 않다. d5 데이터로는 두 값을 <b>가를 수 없다</b>
         * — 런 변동이 효과보다 크다. 판정은 d8 이후로 미룬다.
         *
         * <p><b>1.0/2.0/3.5/6.0 → 1.5/2.8/4.5/7.5</b>(2차 인상). 1차 인상의 실측(시드11 d8)이
         * 방향을 확인해 주었기 때문이다:
         * <pre>
         *   1인당 저장고  6.2 → 4.8 (−23%)   ·  소작 21.4 → 15.2 (−29%)
         *   인구 52 동일 · 미성년/성인여 0.70 → <b>0.78</b> (출산 무손상)
         *   징수 12일 1건 → 8일 80건 · 못냄 1건(소멸 경로 발동 확인)
         *   덤: 수용 인원을 안 건드렸는데 <b>중형이 2채</b> 등장 — 등급별 유지비 차이가
         *       비로소 뜻을 얻어 과시 이사 판정이 실제로 돌기 시작했다
         * </pre>
         * 남은 것은 <b>무토지 9.7 → 9.9 로 거의 안 움직였다</b>는 점이다. 정원이 여전히 바닥을
         * 받치고 있어, 아래 등급을 더 세게(1.0→1.5, 2.0→2.8) 올린다. 저택·대형은 지주가 막사·
         * 시설의 자금원이라 폭을 작게 둔다(6.0→7.5).
         *
         * <p><b>소모 인상 대신 이쪽을 쓰는 이유</b>: 소모는 {@code canReproduce} 에 항으로
         * 들어가 출산 문턱과 도달 속도에 <b>두 번</b> 물린다(실측 철회: 소모 ×1.2 에서 d3 인구
         * 34→29 · 미성년 14→9 인데 1인당 저장고는 4.2→4.9 로 <b>올랐다</b> — 잉여가 아니라
         * 출산이 깎였다). 유지비는 저장고만 깎아 속도에만 한 번 물리고, 정산 순서상 출산 판정이
         * 유지비보다 먼저라 그날 태어날 아이를 뺏지도 않는다.
         */

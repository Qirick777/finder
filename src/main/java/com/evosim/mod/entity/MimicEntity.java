...
public class MimicEntity extends PathfinderMob {
    ...
    private void regenTick() {
        ...
        // <b>전투불가 병사는 스스로 낫지 못한다 — 보급만이 일으켜 세운다.</b>
        //
        // 종전에는 아군 막사 곁에서만 낫게 했는데, 실전에서 재보니 야전병원이 너무 멀었다:
        // 전투 49~62건에 후송 <b>1건</b>이고, 퇴각(체력↓) 9~17건이 나는데 대부분 막사에
        // 닿기 전에 죽었다. 위치를 조건으로 두면 부상병에게 적진 돌파를 요구하게 된다.
        //
        // 위치를 보지 않고 <b>보급</b>만 본다. 지배자가 보낸 사람이 닿아야 일어난다
        // ({@code FarmTicker.supply} 가 복귀선까지 올린다). 그러면
        //   지갑이 마르면 → 보급이 끊기고 → 전선이 못 일어나고 → 전투 가능 병사 0 → 점령
        // 이 한 줄로 이어져, "지배자의 지갑이 곧 전투지속력"이 비로소 두껍게 물린다.
        //
        // 평시 미믹과 성한 병사의 회복은 그대로다 — 퇴각선(30%) 아래의 배속 병사만이다.
        if (treating && FarmTicker.isSoldier(this)) {
            return;
        }
        if (holding <= 0.0) {
    ...
    /**
     * 밭 몸통 = 타일 + 그 사이 고랑. 정본은 {@link FarmStore#bodyOf} 하나다.
     *
     * <p>종전에는 같은 알고리즘이 여기와 {@code FarmStore.bodyColumns} 두 곳에 복사돼 있었고,
     * 축 대응을 여기에만 넣은 탓에 전치 구획의 고랑이 도로 장애물에서 빠졌다.
     */
    private static java.util.Set<Long> bodyOf(FarmStore.Plot plot) {
        return FarmStore.bodyOf(plot);
    }

...
public class MimicFarmGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (mob.inPoorhouse() || FarmTicker.isPastor(mob) || FarmTicker.isFullTimeTeacher(mob)
                || FarmTicker.isAcademic(mob)) { // 교수·대학생 — 수업이 출근이다
            return false; // 경비대·목사는 밭일이 없다(목사 = 전업 성직, 교회 고도화)
        }

...
public final class EvoTest {
    ...
    private static void lending(Report report) {
        ...
        report.add("lending/지대상납", t1, "지대 5·빚 10 → 1 · 2 → 0 · 20 → 4 · 빚 0 → 0 · 빚 2.5 → 3(빚 올림 이하)",
                t1 ? "정상" : "어긋남");
        boolean cap = com.evosim.core.FarmEconomy.C_BASE == 8
                && com.evosim.core.FarmEconomy.capacity(one(Sex.MALE), LifeStage.ADULT) == 8
    ...
    private static void multiplier(Report report) {
        ...
        // ── 경비대 봉급 협상 ──────────────────────────────────────────────
        //
        // 채용 자격이 굶주림에서 <b>요구 봉급</b>으로 옮겨 왔다(FarmTicker.guardAskWage).
        // 요구는 바깥벌이(채집 하루치 × gather)에서 나오므로 채집 배율이 곧 신분을 가른다.
        checkNum(report, "vocation/임금계수무성향", 1.0,
                com.evosim.core.Vocation.guardWageFactor(one(Sex.MALE)),
                "성향 없음 → 요구 그대로");
        checkNum(report, "vocation/임금계수게으름", 0.8,
                com.evosim.core.Vocation.guardWageFactor(
                        one(Sex.MALE, TraitInstance.of(Trait.LAZY))),
                "게으름 → 편한 자리를 싸게 받는다(요구 −20%)");
        checkNum(report, "vocation/임금계수낙관", 1.2,
                com.evosim.core.Vocation.guardWageFactor(
                        one(Sex.MALE, TraitInstance.of(Trait.OPTIMIST))),
                "낙관 → 더 받겠다고 버틴다(요구 +20%)");
        checkNum(report, "vocation/임금계수상쇄", 1.0,
                com.evosim.core.Vocation.guardWageFactor(one(Sex.MALE,
                        TraitInstance.of(Trait.LAZY), TraitInstance.of(Trait.OPTIMIST))),
                "게으름+낙관 = 상쇄");
        boolean vocGate = com.evosim.mod.entity.Facilities.GUARD_WAGE_MAX
                        < com.evosim.mod.entity.Facilities.GUARD_OUTSIDE_DAY
                && Multipliers.gather(one(Sex.MALE, TraitInstance.graded(Trait.PLANT_CONFUSED, 5)))
                        * com.evosim.mod.entity.Facilities.GUARD_OUTSIDE_DAY
                        <= com.evosim.mod.entity.Facilities.GUARD_WAGE_MAX
                && Multipliers.gather(one(Sex.MALE))
                        * com.evosim.mod.entity.Facilities.GUARD_OUTSIDE_DAY
                        > com.evosim.mod.entity.Facilities.GUARD_WAGE_MAX;
        ...
        boolean vocSoldier = com.evosim.core.Vocation.soldier(one(Sex.MALE,
                ...
                && com.evosim.core.Vocation.guardWageFactor(one(Sex.MALE,
                        TraitInstance.of(Trait.BRAVE))) == 1.0;
    ...
    private static void beggar(Report report) {
        ...
        boolean appetite = close(Physique.appetite(graded(Sex.MALE, Trait.STRONG, 5)), 1.25)
                && close(Physique.appetite(brute), 1.125)
                && close(Physique.appetite(feralOnly), 1.0); // 야성은 소모에 안 얹힌다
        report.add("beggar/소모절반", appetite,
                "단순무식Ⅴ 소모 1.125(힘센Ⅴ 1.25 의 절반폭) · 야성은 소모 무관",
    ...
    private static void physique(Report report) {
        ...
        // 2b) 힘 축 트레이드오프: 공격 힘센 +8%/약함 −6%/등급 ↔ 소모(식욕) ±5%/등급.
        //     치명성 인상(4%→5%)으로 힘센Ⅴ 는 소모 1.25 — 큰 몸이 굶을 수 있어야 한다.
        //     튼튼/빈약 ±3%/등급도 함께 확인(종전엔 튼튼이 대가 없는 순이득이었다).
        boolean str = close(Physique.strength(graded(Sex.MALE, Trait.STRONG, 5)), 1.40)
                ...
                && close(Physique.appetite(graded(Sex.MALE, Trait.STRONG, 5)), 1.25)
                && close(Physique.appetite(graded(Sex.MALE, Trait.STRONG, 3)), 1.15)
                && close(Physique.appetite(graded(Sex.MALE, Trait.WEAK, 5)), 0.75)
                && close(Physique.appetite(graded(Sex.MALE, Trait.TOUGH, 5)), 1.15)
                && close(Physique.appetite(graded(Sex.MALE, Trait.FRAIL, 5)), 0.85)
                && close(Physique.appetite(one(Sex.MALE)), 1.0);
        report.add("physique/힘트레이드오프", str,
                "공격 힘센V 1.40·I 1.08·약함V 0.70 ↔ 소모 힘센V 1.25·III 1.15·약함V 0.75 · 튼튼V 1.15·빈약V 0.85",
                str ? "정상" : "어긋남");
    ...
    private static void polygyny(Report report) {
        ...
        boolean g2 = Polygyny.canAccept(java.util.List.of(stingy), 45.0, need, greedyMan)    // 욕심: 질투 무시
                && Polygyny.canAccept(java.util.List.of(stingy, competitive, stingy), 99.0, need, ambitiousMan) // 야망: 4처째도
                && !Polygyny.canAccept(java.util.List.of(stingy), 41.0, need, greedyMan)     // 부유층도 부양은 필수
                // 중혼 문턱(×6) > 개간 임계(18+2×소모) 엄격 성립 — 최소 가구(빈둥지 6.0)에서도
                && need * Polygyny.SUPPORT_DAYS > 18.0 + 2.0 * need
                && 6.0 * Polygyny.SUPPORT_DAYS > 18.0 + 2.0 * 6.0
    ...
    private static void farm(Report report) {
        ...
        //    (12 로 올렸다 되돌림 — FarmEconomy.C_BASE 주석: 지주 자가 수확이 소작을 몰아낸다.)
        Individual man = one(Sex.MALE);
